...
const localIntentJourney = async ({ locale, start, find, confirm, end, query, localText, widths, name }) => {
  ...
  await waitFor(`document.querySelectorAll('.state-panel').length === 0`)
  ...
  if (!await evaluate(`document.querySelector('h1')?.textContent === ${JSON.stringify(locale === 'en' ? 'How can I help you?' : locale === 'hi' ? 'आपको किस काम में मदद चाहिए?' : 'നിങ്ങൾക്ക് എന്ത് സഹായമാണ് വേണ്ടത്?')}`)) throw new Error(`Conversation surface changed during readiness: ${locale}`)
  await clickText(end)
  await waitFor(`Boolean(document.querySelector('#agent-title'))`)
}
...
await waitFor(`document.querySelectorAll('.state-panel').length === 0`)
await evaluate(`document.querySelector('[data-action=prepare]').click()`)
await waitFor(`document.querySelector('h1')?.textContent === 'How can I help you?'`)
...
await waitFor(`Boolean(document.querySelector('.conversation-result')) && Boolean(document.querySelector('.prepared-guidance')) && document.body.textContent.includes('DEMO locality, Bengaluru, Karnataka')`)
await waitFor(`!document.querySelector('#preparation-value') && !document.querySelector('.activity') && Boolean(document.querySelector('.draft-download input'))`)
await evaluate(`document.querySelector('.draft-download input:not(:checked)')?.click()`)
await waitFor(`Boolean(document.querySelector('.official-ready a'))`)
...
await waitFor(`!document.querySelector('.draft-download [role="status"]') || !document.querySelector('.draft-download [role="status"]').textContent.includes('Preparing')`)
await evaluate(`document.querySelector('.draft-download input:not(:checked)')?.click()`)
await waitFor(`Boolean(document.querySelector('.draft-download a[download]'))`)
const draftBase64 = await evaluate(`fetch(document.querySelector('.draft-download a').href).then(response => response.arrayBuffer()).then(bytes => { let text = ''; for (const byte of new Uint8Array(bytes)) text += String.fromCharCode(byte); return btoa(text) })`)
await writeFile(`${outputDir}/prepared-en.pdf`, Buffer.from(draftBase64, 'base64'))
const printed = await send('Page.printToPDF', { printBackground: true, preferCSSPageSize: true })
await writeFile(`${outputDir}/printed-en.pdf`, Buffer.from(printed.data, 'base64'))
...
if (!await evaluate(`getComputedStyle(document.querySelector('.journey-summary')).position === 'static'`)) throw new Error('Journey summary floats')
await evaluate(`window.scrollTo(0, document.body.scrollHeight)`)
if (!await evaluate(`document.querySelector('.journey-summary').getBoundingClientRect().bottom < 0`)) throw new Error('Journey summary did not scroll away')
await evaluate(`document.querySelector('[data-action=procedure]').click()`)
...
await clickText('End session')
await waitFor(`Boolean(document.querySelector('#agent-title')) && document.body.textContent.includes('cleared all in-memory session data')`)
...
await setLocale('hi')
await waitFor(`document.querySelectorAll('.state-panel').length === 0`)
await clickText('सभी सेवाएँ देखें')
await waitFor(`document.querySelectorAll('.service-card').length === 2`)
await evaluate(`document.querySelector('.service-card').click()`)
await waitFor(`Boolean(document.querySelector('[data-action=procedure]'))`)
...
await setLocale('ml')
await waitFor(`document.querySelectorAll('.state-panel').length === 0`)
await clickText('എല്ലാ സേവനങ്ങളും കാണുക')
await waitFor(`document.querySelectorAll('.service-card').length === 2`)
await evaluate(`document.querySelector('.service-card').click()`)
await waitFor(`Boolean(document.querySelector('[data-action=procedure]'))`)
await waitFor(`document.body.textContent.includes('GroqCloud') && document.body.textContent.includes('Groq Console')`)
