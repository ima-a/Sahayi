...
await waitFor(`Boolean(document.querySelector('[data-action=procedure]'))`)
if (!await evaluate(`document.querySelector('.journey-summary').textContent.includes('Aadhaar') && !document.querySelector('.agent-transcript .message') && !document.querySelector('.agent-response')`)) throw new Error('Prior service conversation survived context switch')
