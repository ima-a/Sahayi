...
To enable the optional agent later, the owner must create a restricted Groq project and key, confirm that the account has permission for the exact Enterprise-listed `llama-3.3-70b-versatile` model, and place the key only in Render's secret manager. In Groq Console, the organization owner must explicitly review Data Controls and decide whether to enable Zero Data Retention; Sahayi cannot do this in code, and usage metadata is still documented as collected. Set project spend/rate limits, keep provider/model on their allowlisted values, set `SAHAYI_AGENT_ENABLED=true`, and manually deploy a verified commit. Never put the key in Blueprint YAML, public config, frontend variables, logs, chat, or Git. The process-local limiter and budget reduce demo cost but are not distributed production abuse protection. A native-language review and a separately authorized live evaluation plan are still required before representing AI mode as production-ready.

Groq tier limits are external and may be too low for a public demo; 429 responses intentionally fall back to deterministic guidance. Confirm current limits in Groq Console and official rate-limit documentation rather than relying on repository defaults.
