...
The multi-stage `Dockerfile` uses the project's verified Python 3.14.7 and Node 25.2.1 versions. The Node stage runs `npm ci` and the existing Vite build. The Python stage installs only the project's runtime dependencies. The final image contains the installed backend under `/app/src`, the compiled frontend under `/app/frontend/dist`, form manifests under `/app/form-registry`, and active Procedure Packs under `/app/procedure-packs/packs`; this preserves the application's `__file__`-relative paths without depending on the process working directory. It runs as an unprivileged user and starts one Uvicorn process on `0.0.0.0`, using Render's `PORT` or local fallback `10000`.
...
The frontend build now validates `form-registry/`; copy it into both build and runtime stages. Primary browser smoke starts directly in chat and checks local worksheet PDF download/print. No source PDFs are retrieved at runtime.
