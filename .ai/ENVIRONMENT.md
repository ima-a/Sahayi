...
`SAHAYI_DEV_FRONTEND_ORIGIN` permits one exact Vite development origin. `VITE_API_BASE_URL` selects the frontend development API base. Optional server-only AI configuration is `OPENAI_API_KEY`, `SAHAYI_AGENT_ENABLED` (default false), fixed/allowlisted `SAHAYI_AGENT_MODEL=gpt-5.6-luna`, and bounded timeout, output, tool-round, concurrency, request-budget, and rate-window variables documented as placeholders in `.env.example`. Invalid bounds clamp to safe ranges and any other model value falls back to Luna. The public API exposes only a boolean availability flag.
