...
- Release architecture: browser-local deterministic phrase + Naive Bayes intent matching; same-origin React/FastAPI; deterministic Procedure Packs/readiness/checklists; consent-gated optional OpenAI; offline one-shot source monitoring; synthetic form/submission/status only.
...
- Do not describe translations as certified, model metrics as real-world accuracy, source monitoring as continuous, `store:false` as Zero Data Retention, or demo journeys as real submission/status.
